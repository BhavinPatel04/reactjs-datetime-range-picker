import moment, { type DurationInputArg1, type DurationInputArg2, type Moment } from "moment";
import { type CALENDAR_SIDES, type DATETIME_RANGE_TYPE, type TZ_MAP_KEYS } from "./types";
import { DEFAULT_DATE_FORMAT, NgxDatetimeRangePickerConstants as Constants } from "./constants";
import { getNotAvailableText, cloneDeep, isNil } from "./util";
import {
  type Options,
  type Settings,
  type CalendarSides,
...
export class NgxDatetimeRangePickerService {
  ...
  formatDateToDefaultFormat(date: string | number, format: string): string | undefined {
  ...
  formatTimeToDefaultFormat(time: string): string | null {
  ...
  getCalendarRowNumberText(type: string, number: number): string | undefined {
    return (() => {
      switch (type) {
        case "daily":
          return `W${number}`;
  ...
  createDefaultRanges(config: Config): Record<string, any> {
    const ranges: Record<
      string,
      {
  ...
  getNumberOfWeeks(date: string): number | undefined {
    if (!date) {
      return;
    }
  ...
  getYearlyWeekCount(year: string): number | undefined {
    if (!year) {
      return;
    }
  ...
  getMonthsAvailable(minDate: string, maxDate: string, selectedYear: number): string[] | undefined {
    const months: string[] = [];
  ...
  isDateToday(dateMs: number, config: Config): boolean {
  ...
  getZoneDate(tz: TZ_MAP_KEYS, format: string, date?: string): Moment {
  ...
  getZoneToday(tz: TZ_MAP_KEYS, viewDateFormat: string): string {
    const today: Moment = this.getZoneDate(tz, viewDateFormat);
    return moment(today).format(`${viewDateFormat}  hh:mm A`);
  }

  formatToZoneDate(tz: TZ_MAP_KEYS, format: string, date: string): string {
  ...
  iterateOverDateObj(dates: CalendarSides, func: (arg: ActiveItemSide) => void): void {
  ...
  getCalendarColspan(type: string): number | undefined {
    if (type === "daily") {
      return 6;
    } else if (type === "weekly") {
      return 8;
  ...
  getCalendarRowItemColspan(type: string): number | undefined {
    if (type === "monthly") {
      return 3;
    } else if (type === "quarterly") {
      return 6;
